...
| Packaging, desktop/MIME, device/sandbox access | Implemented metadata/tarball/AppImage/desktop/MIME guidance and fail-closed validation runner | Task 15C/25 tests; Task 24 `scripts/validate-release-hardware.sh` | Clean-install with serial hardware not performed because no controller is attached. AppImage is portable, not sandboxed, and still uses host serial permissions. | Release blocker | Run the validation runner with a readable/writable serial controller and record manual GRBL workflow evidence. |
